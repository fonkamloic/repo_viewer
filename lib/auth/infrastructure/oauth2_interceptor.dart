...
class Oauth2Interceptor extends Interceptor {
  ...
  Future<void> onRequest(
    ...
    final modifiedOptions = await options
      ..headers.addAll(
        credentials == null
            ? {}
  ...
  Future<void> onError(DioError err, ErrorInterceptorHandler handler) async {
    ...
    if (errorResponse != null && errorResponse.statusCode == 401) {
      ...
      final refeshedCredentials = await _authenticator.getSignedInCredentials();
      if (refeshedCredentials != null) {
        handler.resolve(await _dio.fetch(
          errorResponse.requestOptions
            ..headers['Authorization'] =
                'bearer ${refeshedCredentials.accessToken}',
        ));
      }
    } else {
      handler.next(err);
